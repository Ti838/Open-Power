# Open Power Research Preview API

REST API exposing the **ULTRAPLINIAN** multi-model racing engine and core Open Power systems: **AutoTune** (context-adaptive LLM parameter tuning), **Parseltongue** (text obfuscation), **STM** (semantic text transformation), **Feedback Loop** (EMA-based parameter learning), and **opt-in open dataset collection**.
...
# Docker
docker build -t open-power-api .
docker run -p 7860:7860 open-power-api

# With auth enabled
docker run -p 7860:7860 -e GODMODE_API_KEY=your-secret-key open-power-api
```
...
const completion = await client.chat.completions.create({
  model: 'nousresearch/hermes-3-llama-3.1-70b',
  messages: [{ role: 'user', content: 'Hello!' }],
  // @ts-ignore — Open Power extension field
...
The Open Power pipeline (GODMODE, AutoTune, Parseltongue, STM) runs transparently behind the standard interface. Pipeline metadata is returned in the `x_open-power` extension field (ignored by standard SDKs). To disable the pipeline, pass `godmode: false, autotune: false, parseltongue: false, stm_modules: []`.
...
The response is standard OpenAI format. The winning model name is in `response.model`. Race metadata is in `x_open-power.race`.
...
# x_open-power.orchestrator.model → the synthesizer (e.g. "anthropic/claude-sonnet-4")
```
...
```json
{
  "object": "list",
  "data": [
    {"id": "ultraplinian/fast", "object": "model", "created": 1700000000, "owned_by": "open-power"},
    {"id": "ultraplinian/standard", "object": "model", "created": 1700000000, "owned_by": "open-power"},
    {"id": "ultraplinian/full", "object": "model", "created": 1700000000, "owned_by": "open-power"},
...
| `godmode` | bool | no | `true` | Inject GODMODE system prompt (Open Power extension) |
| `autotune` | bool | no | `true` | Enable AutoTune (Open Power extension) |
| `parseltongue` | bool | no | `true` | Enable Parseltongue (Open Power extension) |
| `stm_modules` | array | no | `["hedge_reducer", "direct_mode"]` | STM modules (Open Power extension) |
| `contribute_to_dataset` | bool | no | `false` | Opt in to dataset (Open Power extension) |
...
- API keys (OpenRouter or Open Power)
- IP addresses
- Auth tokens
...
huggingface-cli upload pliny-the-prompter/open-power dataset.jsonl
```
...
Read-access endpoints for the [`pliny-the-prompter/open-power`](https://huggingface.co/datasets/pliny-the-prompter/open-power) HuggingFace dataset. Query, filter, and download the full published corpus — not just the current in-memory buffer.
...
```json
{
  ...
  "repo": "pliny-the-prompter/open-power",
  "enabled": true
}
```
...
with open("open-power-dataset.jsonl", "w") as f:
    f.write(r.text)
...
with open("open-power-corpus.jsonl", "wb") as f:
...
| `HF_DATASET_REPO` | Target HF dataset repo | `pliny-the-prompter/open-power` |
