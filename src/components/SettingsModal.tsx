...
export function SettingsModal({ onClose }: SettingsModalProps) {
  ...
  const tabs: { id: SettingsTab; icon: React.ReactNode; label: string }[] = [
    { id: 'api',         icon: <Key className="w-4 h-4" />,             label: t('settings.apiKey') },
    { id: 'prompt',      icon: <FileText className="w-4 h-4" />,         label: t('settings.systemPrompt') },
    { id: 'autotune',    icon: <SlidersHorizontal className="w-4 h-4" />,label: t('settings.autotune') },
    { id: 'appearance',  icon: <Palette className="w-4 h-4" />,          label: t('settings.appearance') },
    { id: 'privacy',     icon: <Shield className="w-4 h-4" />,           label: t('settings.privacy') },
    { id: 'liquid',      icon: <Droplets className="w-4 h-4" />,         label: t('settings.liquid') },
    { id: 'parseltongue',icon: <Layers className="w-4 h-4" />,           label: t('settings.stealth') },
    { id: 'stm',         icon: <Zap className="w-4 h-4" />,              label: t('settings.stm') },
    { id: 'memory',      icon: <Sparkles className="w-4 h-4" />,         label: t('settings.memory') },
    { id: 'ultraplinian',icon: <Swords className="w-4 h-4" />,           label: t('settings.speedRace') },
    { id: 'consortium',  icon: <Users className="w-4 h-4" />,            label: t('settings.multiModel') },
    { id: 'plan',        icon: <Crown className="w-4 h-4" />,            label: t('settings.plan') },
    { id: 'data',        icon: <Download className="w-4 h-4" />,         label: t('settings.data') },
  ]
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative flex h-[85vh] w-full max-w-2xl overflow-hidden rounded-[28px] border border-white/[0.09] bg-[var(--bg-panel)]/95 shadow-[0_40px_120px_-20px_rgba(0,0,0,0.9)] backdrop-blur-2xl slide-up">

        {/* Left nav */}
        <nav className="flex w-52 shrink-0 flex-col border-r border-white/[0.07] bg-white/[0.02] p-3">
          {/* Header logo */}
          ...
          <div className="flex-1 space-y-0.5 overflow-y-auto">
            {tabs.map((tab) => (
              ...
              >
                <span className={activeTab === tab.id ? 'text-sky-300' : 'text-slate-500'}>{tab.icon}</span>
                {tab.label}
              </button>
            ))}
          </div>
          ...
          <button
            onClick={onClose}
            className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-white/[0.07] bg-white/[0.03] py-2 text-sm text-slate-400 transition hover:bg-white/[0.07] hover:text-white"
          >
            <X className="h-4 w-4" />
            Close
          </button>
...
function APIKeyTab() {
  ...
  const [generatedLocalKey, setGeneratedLocalKey] = useState('')
  ...
  const createLocalAccessKey = () => {
    const random = Math.random().toString(36).slice(2, 10)
    const generated = `op_local_${Date.now().toString(36)}_${random}`
    const apiBase = getLocalApiBase()

    setUltraplinianApiUrl(apiBase)
    setUltraplinianApiKey(generated)
    setGeneratedLocalKey(generated)
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-lg font-semibold mb-2">Default Model</h3>
        <p className="text-sm theme-secondary mb-3">
          Choose the model used by default for standard chat.
        </p>
        <ModelSelector />
      </div>
      ...
      <div>
        ...
        <p className="text-sm theme-secondary mb-4">
          Your API key is stored locally and never sent to any Open Power server.
          Get your key at{' '}
          <a
      ...
      <div className="rounded-lg border border-sky-400/30 bg-sky-400/10 p-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-sky-200">No OpenRouter account?</p>
            <p className="text-xs text-sky-200/80">
              Generate a local access key for self-hosted API mode.
            </p>
          </div>
          <button
            onClick={createLocalAccessKey}
            className="rounded-lg border border-sky-400/35 bg-sky-400/20 px-3 py-1.5 text-xs font-semibold text-sky-100 transition hover:bg-sky-400/30"
          >
            Generate Local Key
          </button>
        </div>
        {generatedLocalKey && (
          <p className="mt-2 truncate font-mono text-[11px] text-sky-200">
            Created: {generatedLocalKey}
          </p>
        )}
        <p className="mt-2 text-[11px] text-sky-200/70">
          Note: This key authenticates to your local Open Power API only. The API server still needs an upstream model key (OPENROUTER_API_KEY) unless you configure another provider backend.
        </p>
      </div>
    </div>
  )
...
function SystemPromptTab() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-lg font-semibold mb-1">Open Power System Prompt</h3>
        <p className="text-sm theme-secondary mb-4">
    ...
    <div className="space-y-5">
      ...
      {parseltongueConfig.enabled && (
        <>
          ...
          <div className="p-4 bg-theme-dim border border-theme-primary rounded-lg">
            <div className="flex items-start gap-3">
              <StealthIcon className="w-5 h-5 text-green-400 flex-shrink-0 mt-0.5" />
              <div className="text-sm">
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <div className="p-3 rounded-lg border border-fuchsia-500/30 bg-fuchsia-500/5">
                <div className="flex items-center gap-2 mb-1">
                  <StealthIcon className="w-3 h-3 text-fuchsia-400" />
              ...
              <div className="p-3 rounded-lg border border-green-500/30 bg-green-500/5">
                <div className="flex items-center gap-2 mb-1">
                  <StealthIcon className="w-3 h-3 text-green-400" />
...
function AppearanceTab() {
  const { theme, setTheme, language, setLanguage, showMagic, setShowMagic } = useStore()

  const themes: { id: Theme; name: string; nameBn: string; dot: string; description: string }[] = [
    { id: 'light',    name: 'Light',    nameBn: 'লাইট',      dot: '#2563eb', description: 'Bright canvas with clean contrast' },
    { id: 'power',    name: 'Power',    nameBn: 'পাওয়ার',    dot: '#3b82f6', description: 'Electric blue on deep navy' },
    { id: 'dark',     name: 'Obsidian', nameBn: 'অবসিডিয়ান', dot: '#e2e8f0', description: 'Pure dark, minimal contrast' },
    { id: 'sunset',   name: 'Sunset',   nameBn: 'সূর্যাস্ত', dot: '#f97316', description: 'Warm amber on dark brown' },
    { id: 'forest',   name: 'Forest',   nameBn: 'বন',        dot: '#22c55e', description: 'Deep green nature vibes' },
    { id: 'ocean',    name: 'Ocean',    nameBn: 'সমুদ্র',    dot: '#06b6d4', description: 'Deep blue-teal serenity' },
    { id: 'sakura',   name: 'Sakura',   nameBn: 'সাকুরা',    dot: '#f472b6', description: 'Soft pink blossom tones' },
    { id: 'midnight', name: 'Midnight', nameBn: 'মধ্যরাত',   dot: '#818cf8', description: 'Deep indigo dream palette' },
    { id: 'terminal', name: 'Terminal', nameBn: 'টার্মিনাল',  dot: '#00ff41', description: 'Classic green on black' },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-sm text-white">Interface language / ভাষা</h3>
            <p className="text-xs text-slate-500 mt-0.5">Switch between English and Bengali for the full UI.</p>
          </div>
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                language === 'en'
                  ...
                  : 'border border-white/[0.08] text-slate-400 hover:bg-white/[0.05] hover:text-white'
              }`}
            >English</button>
            <button
      ...
      <div>
        ...
        <p className="text-xs opacity-50 mb-3">
          Pick the visual style for the app shell.
        </p>
      </div>
          ...
          <button
            ...
            className={`p-3 rounded-xl border text-left transition-all duration-200 group
              ${theme === t.id
                ...
                : 'border-[var(--border,rgba(255,255,255,0.08))] hover:border-[var(--primary)]/40 hover:bg-[var(--accent)]/50'
              }`}
          >
            <div className="flex items-center gap-2 mb-1.5">
              ...
              {theme === t.id && (
                <span className="text-[9px] font-mono bg-[var(--primary)]/15 text-[var(--primary)] px-1.5 py-0.5 rounded">
                  ON
                </span>
              )}
            </div>
            <p className="text-[10px] opacity-50 leading-tight pl-5">{t.description}</p>
          </button>
        ))}
      </div>

      <div className="pt-4 border-t border-[var(--border,rgba(255,255,255,0.08))]">
        <ToggleSetting
          label="Show advanced message details"
          description="Display model tags, AutoTune details, feedback buttons, and race response arrows inside messages."
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm theme-secondary">
          Open Power respects your privacy. No data is sent anywhere except to the model providers you choose.
        </p>
      </div>
...
function UltraplinianTab() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-lg font-semibold mb-1">Speed Race Mode</h3>
        <p className="text-sm theme-secondary mb-4">
          Race multiple AI models in parallel. The fastest, best-scored response wins.
        </p>
      </div>

      {/* Master toggle */}
      <ToggleSetting
        label="Enable Speed Race"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                ...
                <button
                  onClick={generateUltraplinianKey}
                  className="rounded border border-theme-primary/40 px-1.5 py-0.5 text-[10px] font-mono theme-primary transition hover:bg-theme-accent"
                  title="Generate a local API key"
                >
                  GEN
                </button>
                <button
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm">
                ...
                <ul className="space-y-1 theme-secondary text-xs">
                  ...
                  <li>* Open Power prompt + Depth Directive injected automatically</li>
...
function ConsortiumTab() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-lg font-semibold mb-1">Multi-Model Mode</h3>
        <p className="text-sm theme-secondary mb-4">
        ...
        <div className="p-4 bg-theme-dim border border-theme-primary rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm">
              ...
              <p className="theme-secondary text-xs">
                API key tiers are assigned server-side via the legacy <code className="font-mono text-[10px] bg-theme-accent px-1 rounded">GODMODE_TIER_KEYS</code> environment variable. Contact the API host or set the variable yourself if self-hosting.
              </p>
            </div>
          </div>
        </div>
...
function DataTab() {
  ...
  const handleExport = () => {
    ...
    a.href = url
    a.download = `open-power-backup-${new Date().toISOString().split('T')[0]}.json`
    a.click()
    ...
    reader.onload = (event) => {
      try {
        ...
        if (typeof imported !== 'object' || imported === null) {
          throw new Error('Not a valid Open Power export file.')
        }
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-lg border border-theme-primary bg-theme-accent/50">
        ...
        <p className="text-sm theme-secondary leading-relaxed">
          Open Power stores everything locally in this browser — conversations, memories, settings, API keys.
...
function ToggleSetting({
  ...
  return (
    <div className="flex items-start justify-between gap-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4 transition-all hover:border-white/[0.12]">
      <div className="flex-1">
      ...
      <button
        ...
        className={`relative h-6 w-11 flex-shrink-0 rounded-full transition-all duration-200 ${
          enabled ? 'bg-sky-500 shadow-[0_0_10px_rgba(56,189,248,0.4)]' : 'bg-white/10'
        }`}
