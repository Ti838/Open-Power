...
// Types
export type Theme = 'light' | 'power' | 'dark' | 'neon' | 'sunset' | 'forest' | 'ocean' | 'sakura' | 'midnight' | 'terminal'
...
const defaultPersonas: Persona[] = [
  {
    id: 'core',
    name: 'Open Power Core',
    description: 'Direct, capable assistant mode with a clean default tone.',
    tone: 'direct, decisive, helpful',
    coreDirective: 'You are Open Power, a focused AI assistant. Answer clearly, directly, and helpfully. Keep responses useful and action-oriented.',
  ...
  persist(
    (set, get) => ({
      ...
      restoreBackup: (data) => set(() => {
        ...
        const allowed = [
          'conversations', 'currentConversationId', 'theme', 'language', 'defaultModel',
        ...
        for (const key of allowed) {
          if (key in data && data[key] !== undefined) {
            if (key === 'theme') {
              const oldTheme = data[key] as string
              const validThemes: Theme[] = ['power','dark','neon','sunset','forest','ocean','sakura','midnight','terminal']
              if (['matrix', 'glyph'].includes(oldTheme)) {
                next[key] = 'neon'
              } else if (oldTheme === 'hacker') {
                next[key] = 'dark'
              } else if (oldTheme === 'minimal') {
                next[key] = 'dark'
              } else if (oldTheme === 'light') {
                next[key] = 'dark'
              } else if (validThemes.includes(oldTheme as Theme)) {
                next[key] = data[key]
              } else {
                next[key] = 'power'
              }
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          if (state.defaultModel === 'anthropic/claude-opus-4.6') {
            state.setDefaultModel('openai/gpt-4o-mini')
          }
          state.setHydrated()
        }
      }
