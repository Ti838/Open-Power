...
const corsOrigins = process.env.CORS_ORIGIN === '*'
  ...
  : [process.env.CORS_ORIGIN || 'https://openpower.app', ...(process.env.HF_SPACE_URL ? [process.env.HF_SPACE_URL] : [])].filter(Boolean)
...
app.get('/v1/info', (_req, res) => {
  res.json({
    name: 'Open Power Research Preview API',
    version: '0.4.0',
    ...
    source: 'https://github.com/LYS10S/Open Power',
  })
})
...
app.get('/v1/models', (_req, res) => {
  ...
  const virtualModels = [
    { id: 'ultraplinian/fast', owned_by: 'open-power' },
    { id: 'ultraplinian/standard', owned_by: 'open-power' },
    { id: 'ultraplinian/smart', owned_by: 'open-power' },
    { id: 'ultraplinian/power', owned_by: 'open-power' },
    { id: 'ultraplinian/ultra', owned_by: 'open-power' },
    // CONSORTIUM — hive-mind synthesis from all models
    { id: 'consortium/fast', owned_by: 'open-power' },
    { id: 'consortium/standard', owned_by: 'open-power' },
    { id: 'consortium/smart', owned_by: 'open-power' },
    { id: 'consortium/power', owned_by: 'open-power' },
    { id: 'consortium/ultra', owned_by: 'open-power' },
  ]
...
app.get('/v1/tier', apiKeyAuth, (req, res) => {
  ...
  if (!config) {
    res.status(500).json({ error: 'Tier configuration missing' })
    return
  }
  res.json({
    tier: config.name,
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`
  ╔══════════════════════════════════════════════════════════╗
  ║  Open Power Research Preview API v0.4.0                     ║
