 ...
 * Research API Routes
 *
 * Read-access endpoints for the pliny-the-prompter/open-power HuggingFace
...
researchRoutes.get('/info', (_req, res) => {
  res.json({
    name: 'Open Power Research Dataset',
    repo: 'pliny-the-prompter/open-power',
    repo_url: 'https://huggingface.co/datasets/pliny-the-prompter/open-power',
    license: 'AGPL-3.0',
...
// Express 5 path parser is strict; regex route avoids path-to-regexp incompatibilities.
...
researchRoutes.get('/download', tierGate('corpus:download'), async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="open-power-corpus-${Date.now()}.jsonl"`)
