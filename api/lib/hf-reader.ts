 ...
 *
 * Reads published batch files back from the pliny-the-prompter/open-power
...
function getConfig() {
  return {
    ...
    repo: process.env.HF_DATASET_REPO || 'pliny-the-prompter/open-power',
...
function evictStaleCache() {
  const now = Date.now()
  for (const [key, entry] of Array.from(batchContentCache.entries())) {
